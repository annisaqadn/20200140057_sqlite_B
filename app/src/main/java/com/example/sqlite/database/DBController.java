...
public class DBController extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        //Tempat membuat tabel
        db.execSQL("create table teman(id integer primary key, nama text, telpon text)");
    }

    @Override
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        //Jika sqlite versinya di upgrade, tabelnya dihapus terlebih dahulu
        db.execSQL("drop table if exists teman");
        onCreate(db);
    }

    public void insertData(HashMap<String,String> queryValues) { //String pertama adalah kunci, String kedua adalah values
        SQLiteDatabase basisdata = this.getWritableDatabase();
        ContentValues nilai = new ContentValues();
        nilai.put("nama",queryValues.get("nama"));
        nilai.put("telpon",queryValues.get("telpon"));
        basisdata.insert("teman",null, nilai);//Untuk masukkan tabel ke database
        basisdata.close(); //Basisdata ditutup
    }
